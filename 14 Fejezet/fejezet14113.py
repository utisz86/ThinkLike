...
def tesztkeszlet():
    ...
    # compare_list
    teszt(compare_list([2,0,1,3], [2,0,1,3]) == True)
    teszt(compare_list([2,0,1,3], [0,1,2,3]) == False)
    # compare_listin
    teszt(compare_listin([2,0,1,3], [[0,1,2,3],[2,0,1,3]]) == True)
    teszt(compare_listin([2,0,1,3], [[1,2,0,3],[0,1,2,3]]) == False)
...
def compare_list(xs, ys):
    for (i, x) in enumerate(xs):
        if x != ys[i]:
            return False
    return True
    
def compare_listin(xs, ys):
    for (i, y) in enumerate(ys):
        if compare_list(xs, y):
            return True
    return False
...
def main():
    ...
    while talalat_szama < 10:
        ...
        if not van_utkozes(bd):
            if megoldas not in megoldasok:
                megoldasok.append(megoldas[:])
                print("Megoldas: {0}, probalkozas: {1}.".format(bd, proba))
                proba_atlag += proba
                proba = 0
                talalat_szama += 1
    print("Atlag proba: {0}".format(proba_atlag/10))
    print(megoldasok)
